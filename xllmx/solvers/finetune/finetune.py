...
class FinetuneSolverBase(ABC):
    ...
    def __init__(self, args):
        ...
        self.dataset_train, self.sampler_train, self.dataloader_train, self.dataset_val, self.sampler_val, self.dataloader_val = self.build_data_new()
    ...
    def get_args_parser(cls):
        ...
        parser.add_argument("--data_config_val", default="/path/to/data/config/yaml", type=str, help="data config path")
        parser.add_argument(
            "--cache_ann_on_disk",
            action="store_true",
        ...
        parser.add_argument("--eval_only", action="store_true", default=False, help="eval only or not")
        parser.add_argument("--ft", action="store_true", default=False, help="fintune from the pretrained model or not")
    ...
    def build_data_new(self):
        eff_batch_size = self.args.batch_size * self.args.accum_iter * fs_init.get_data_parallel_world_size()
        self.logger.info("effective batch size: %d" % eff_batch_size)
        dataset_train, dataset_val = self._dataset_func_new()
        self.logger.info(dataset_train)
        self.logger.info(dataset_val)
        ...
        sampler_val = FinetuneDistSampler(
            dataset_val,
            num_replicas=self.dp_world_size,
            rank=self.dp_rank,
            shuffle=True,
        ...
        return dataset_train, sampler_train, dataloader_train, dataset_val, sampler_val, dataloader_val
    ...
    def _dataset_func_new(self):
        ...
        dataset_val = FinetuneConversationDataset(
            self.args.data_config_val, item_processor=item_processor, cache_on_disk=self.args.cache_ann_on_disk
        )
    ...
    def resume(self, resume_path: str):
        ...
        if not self.args.eval_only and not self.args.ft:
            _load_optimizer()
    ...
    def run_with_eval(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            if self.global_rank == 0:
                if self.log_writer is not None:
                    self.log_writer.flush()
                with open(os.path.join(self.args.output_dir, "log_eval.txt"), mode="a", encoding="utf-8") as f:
                    f.write(json.dumps(log_stats) + "\n")
    ...
    def run_with_eval_0_i(self):
        self.logger.info(f"Start training for {self.args.epochs} epochs")
        start_time = time.time()
        for epoch in range(self.start_epoch, self.args.epochs):
            self.dataloader_train.sampler.set_epoch(epoch, self.start_iter)  # todo rename set_epoch

            train_stats = self.train_one_epoch_0_i(
                epoch,
                self.start_iter,
                log_writer=self.log_writer,
                metric_logger=self.metric_logger_to_resume,
            ...
            val_stats = self.val_one_epoch_0_i(
                epoch,
                self.start_iter,
                log_writer=self.log_writer,
                metric_logger=self.metric_logger_to_resume,
    ...
    def run_with_eval_awm(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            val_stats = self.val_one_epoch_awm(
                epoch,
                self.start_iter,
                log_writer=self.log_writer,
                metric_logger=self.metric_logger_to_resume,
    ...
    def run_with_eval_awm_w(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            val_stats = self.val_one_epoch_awm_w(
                epoch,
                self.start_iter,
                log_writer=self.log_writer,
                metric_logger=self.metric_logger_to_resume,
    ...
    def run_with_eval_awm_fast(self):
        self.logger.info(f"Start training for {self.args.epochs} epochs")
        start_time = time.time()
        for epoch in range(self.start_epoch, self.args.epochs):
            self.dataloader_train.sampler.set_epoch(epoch, self.start_iter)  # todo rename set_epoch

            train_stats = self.train_one_epoch_awm_fast(
                epoch,
                self.start_iter,
                log_writer=self.log_writer,
                metric_logger=self.metric_logger_to_resume,
            ...
            val_stats = self.val_one_epoch_awm_fast(
                epoch,
                self.start_iter,
                log_writer=self.log_writer,
                metric_logger=self.metric_logger_to_resume,
    ...
    def eval(self):
        self.logger.info(f"Start evaluation...")
    ...
    def eval_g(self):
        ...
        val_stats = self.val_one_epoch_g(
            0,
            0,
            log_writer=self.log_writer,
            metric_logger=self.metric_logger_to_resume,
    ...
    def eval_awm_g(self):
        ...
        val_stats = self.val_one_epoch_awm_g(
            0,
            0,
            log_writer=self.log_writer,
            metric_logger=self.metric_logger_to_resume,
    ...
    def eval_awm_2_g(self):
        self.logger.info(f"Start evaluation...")
            
        val_stats = self.val_one_epoch_awm_2_g(
            0,
            0,
            log_writer=self.log_writer,
            metric_logger=self.metric_logger_to_resume,
    ...
    def eval_awm_f_g(self):
        self.logger.info(f"Start evaluation...")
            
        val_stats = self.val_one_epoch_awm_f_g(
            0,
            0,
            log_writer=self.log_writer,
            metric_logger=self.metric_logger_to_resume,
    ...
    def train_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                c_loss, additional_loss_dict, logits, hidden_states, labels_c = self.model(examples, labels, output_hidden_states=True, training=True)
            loss = c_loss
            ...
            shift_labels = labels_c[..., 1:].contiguous()
            valid_mask = shift_labels >= 0
            valid_logits = shift_logits[valid_mask].reshape(labels_c.shape[0], -1, logits.shape[-1]) # torch.Size([8, 10, 65536])
            action_logits = valid_logits[:,1:-2] # torch.Size([8, 7, 65536])
            dis_action = action_logits.argmax(dim=2) # torch.Size([8, 7])
            conti_action = torch.tensor(self.decode_token_ids_to_actions(dis_action))
            gt_dis_action = shift_labels[valid_mask].reshape(labels_c.shape[0], -1)[:,1:-2]
            gt_conti_action = torch.tensor(self.decode_token_ids_to_actions(gt_dis_action))
            action_l1_loss = torch.nn.functional.l1_loss(conti_action, gt_conti_action)
            metric_logger.update(l1_loss=action_l1_loss)
            
            correct = (dis_action == gt_dis_action).float()
    ...
    def train_one_epoch_0_i(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            examples, labels = batch_data
            examples, labels = self.process_lists(examples, labels)
            ...
            with {
                ...
                c_loss, additional_loss_dict, logits, hidden_states, labels_c = self.model(examples, labels, output_hidden_states=True)
            loss = c_loss
            for add_loss, weight in additional_loss_dict.values():
                loss = loss + add_loss * weight
            loss_value = loss.item()
    ...
    def train_one_epoch_awm_w(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                c_loss, additional_loss_dict, logits, hidden_states, labels_c = self.model(examples, labels, output_hidden_states=True, loss_weights=loss_weights, training=True)
            loss = c_loss
            for add_loss, weight in additional_loss_dict.values():
                loss = loss + add_loss * weight
            loss_value = loss.item()
    ...
    def train_one_epoch_awm_fast(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            accuracies_action, accuracies_image, l1_loss = self.calculate_accuracies_fast(labels_c, logits)
    ...
    def calculate_accuracies_g(self, generation_result_list, gt_result_list):
        acc_list = []
        l1_loss_list = []
        for i in range(len(generation_result_list)):
            generation_result = generation_result_list[i]
            gt_result = gt_result_list[i]
            correct = torch.eq(generation_result, gt_result).float()
            accuracy = correct.mean().item()
            l1_loss = torch.nn.functional.l1_loss(generation_result.float(), gt_result.float()).item()
            acc_list.append(correct)
            l1_loss_list.append(l1_loss)
        return acc_list, l1_loss_list
    
    def calculate_accuracies_g_2(self, labels_bs, preds_bs, output_img=False):
        accuracies_action_bs = []
        accuracies_image_bs = []
        l1_loss_bs = []
        for bs in range(len(labels_bs)):
            labels = labels_bs[bs]
            preds = preds_bs[bs]
        ...
        if output_img:
            ...
            return self.calculate_position_averages(accuracies_action_bs), self.calculate_position_averages(l1_loss_bs)
    ...
    def calculate_accuracies_fast(self, labels_bs, logits_bs):
        accuracies_action_bs = []
        accuracies_image_bs = []
        l1_loss_bs = []
        for bs in range(labels_bs.shape[0]):
            ...
            while i < len(labels):
                ...
                if self.is_action_token(labels[i]):
                    ...
                    conti_action = torch.tensor(self.decode_token_ids_to_actions_fast(pred))
                    gt_conti_action = torch.tensor(self.decode_token_ids_to_actions_fast(labels[start+1:end]))
                    action_l1_loss = torch.nn.functional.l1_loss(conti_action, gt_conti_action)
                    l1_loss.append(action_l1_loss)
    ...
    def val_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            for metric_name, metric in metric_logger.meters.items():
                ...
                if log_writer is not None:
                    log_writer.add_scalar(
                        'val_' + metric_name, metric_value, data_iter_step + len(self.dataloader_val) * epoch
                    )
    ...
    def val_one_epoch_g(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                # generation_result, gt_result = self.model.generate_batch_1(
                #     examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                # )
                generation_result, gt_result = self.model.generate_batch_2(
                    examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                )
            torch.cuda.synchronize()
            ...
            accuracies_action, l1_loss = self.calculate_accuracies_g_2(gt_result, generation_result)
                
            for i in range(len(accuracies_action)):
                metric_logger.update(**{f"acc_action_{i}": accuracies_action[i]})
                metric_logger.update(**{f"l1_loss_action_{i}": l1_loss[i]})
            
            metric_logger.update(lr=0.00002)
    ...
    def val_one_epoch_awm_g(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                #     examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                # )
                generation_result, gt_result = self.model.generate_batch_awm(
                    examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                )
            torch.cuda.synchronize()
    ...
    def val_one_epoch_awm_2_g(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                #     examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                # )
                generation_result, gt_result = self.model.generate_batch_awm_2(
                    examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                )
            torch.cuda.synchronize()
            ...
            accuracies_action, accuracies_image, l1_loss = self.calculate_accuracies_g_2(gt_result, generation_result, output_img=True)
    ...
    def val_one_epoch_awm_f_g(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                #     examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                # )
                generation_result, gt_result = self.model.generate_batch_awm_f(
                    examples, labels, generation_config, logits_processor=logits_processor, streamer=None
                )
            torch.cuda.synchronize()
    ...
    def decode_token_ids_to_actions_fast(self, dis_action):
        decoded_actions = self.tokenizer_fast.decode([dis_action.cpu().numpy().tolist()], action_dim=7, time_horizon=5)
