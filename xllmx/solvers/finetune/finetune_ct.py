...
class FinetuneSolverBase_ContinuousHead(ABC):
    ...
    def get_args_parser(cls):
        ...
        parser.add_argument("--eval_only", action="store_true", default=False, help="enable gradient checkpointing")
    ...
    def train_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                c_loss, loss_ct, additional_loss_dict, logits, hidden_states, labels_c, _ = self.model(examples, labels, training=True, output_hidden_states=True)
            loss = c_loss + loss_ct
            # loss = c_loss
    ...
    def train_one_epoch_awm_w(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                c_loss, loss_ct, additional_loss_dict, logits, hidden_states, labels_c, l1_loss_ct = self.model(examples, labels, output_hidden_states=True, loss_weights=loss_weights)
            loss = c_loss + loss_ct
            for add_loss, weight in additional_loss_dict.values():
                loss = loss + add_loss * weight
            loss_value = loss.item()
            ...
            accuracies_action, accuracies_image, l1_loss_dis = self.calculate_accuracies(labels_c, logits)
                            
            for i in range(len(accuracies_action)):
                metric_logger.update(**{f"acc_action_{i}": accuracies_action[i]})
                metric_logger.update(**{f"l1_loss_dis_action_{i}": l1_loss_dis[i]})
                metric_logger.update(**{f"l1_loss_ct_action_{i}": l1_loss_ct[i]})
    ...
    def val_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            metric_logger.update(closs=c_loss_value)
            metric_logger.update(loss_ct=loss_ct)
            metric_logger.update(**{key: val[0].item() for key, val in additional_loss_dict.items()})
            lr = self.optimizer.param_groups[0]["lr"]
            metric_logger.update(lr=lr)
